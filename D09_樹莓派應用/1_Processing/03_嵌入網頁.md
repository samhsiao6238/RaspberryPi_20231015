...
3. 將 Processing 的 Python 腳本轉換為 P5.js，這個轉換難度不高，有興趣的同學可自行琢磨。這裡主要是展示腳本嵌入網頁的方式，所以不特別介紹 Python 轉換為 P5.js 的過程。
...
4. 建立超文本 `index.html` ，依照 p5.js 腳本內容必須與 `shared_file.txt` 文件置於同層目錄，至於是否與 Processing 腳本放在一起皆可，因為此後進行通信的 Processing 程序會內嵌到超文本中，而不是原本的 Processing 腳本。
...
5. 在超文本 `index.html` 中必須鏈接 p5.js 庫。
...
6. 先看一下文本  `index.html` 的架構，以下範例尚未寫入 p5.js 程式碼。
...
8. 在 VSCode 中可透過 `Live Server` 插件進行測試。

   ![](images/img_23.png)

<br>

9. 會開啟本地網頁，網址是 `127.0.0.1:5500` 。

   ![](images/img_24.png)

<br>

10. 區網訪問使用 `樹莓派網址:5500` ，我的樹莓派網址是 `192.168.1.134` 。
