...
1. 在專案資料夾內建立一個測試用的文件檔案 `shared_file.txt` ，或使用原本的文件，同樣預先寫入內容如 `AABBCCDD` 。
...
3. 建立 `index.html` 文件，依照腳本內容必須與 `shared_file.txt` 文件置於同層目錄，至於是否與 Processing 腳本放在一起皆可。
...
4. 在文本 `index.html` 中必須鏈接 p5.js 庫。
...
7. 在 VSCode 中透過 Live Server 插件進行測試。
