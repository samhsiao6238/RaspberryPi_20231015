...
1. 建立一個基礎的 Tkinter 視窗。

    ```python
    from tkinter import *
    # 初始化Tkinter視窗
    tk_calc = Tk()
    tk_calc.configure(bg="#293C4A", bd=10)  # 設定背景顏色和邊界寬度
    tk_calc.title("科學計算機")  # 設定標題
    tk_calc.mainloop()
    ```
...
3. 建立 0-9 的數字鍵，並且套用前一步驟所設定的顏色等參數，特別注意，程序的索引 `index` 都是以 `0` 作為起始，但以下腳本中的 `row` 是從 `1` 開始，這代表程序保留了 `row 0`，而這正是要給計算機的 `結果顯示欄位` 使用。
    ...
    for i in range(9):
        Button(
            ...
            button_params_main, 
            text=str(i + 1),
            command=lambda num=i + 1: button_click(str(num))).grid(row=(i // 3) + 1, 
            column=i % 3, 
            sticky="nsew"
        )
    ```
...
4. 使用 grid 佈局，依據加入加減乘除四顆按鈕，參數 text 依序為 `+-*/`，`row` 則依序為 `1~4`。
...
8. 這裡注意，雖然 `等號` 的函數 `button_equal` 沒實作會造成程序崩潰，但其他按鍵的 `Lambda` 函數 `button_click` 沒實作卻不會出錯，補充說明。
...
12. 假如想要更改色系，可以直接調整 `button_params_main` 的值，這裡提供一個暗色系的設置。
    ...
    ```python
    {'bd': 5, 'fg': '#BBB', 'bg': '#3C3636', 'font': ('sans-serif', 20, 'bold')}
    ```
    _顯示_
    ![](images/img_12.png)
...
13. 接著加入結果顯示框，在結果運算部分會使用到 textvariable 來綁定 UI 元件，如此便可實現計算結果的動態更新，相關細節詳補充說明，跟函數設置一樣，若不設置會報錯。
...
1. Labda 函數 `button_click` 未實現但不會導致崩潰。
    ...
    - 範例中 `button_click` 函數是通過 `lambda` 函數間接參考的，所以創建按鈕並將 `command` 參數設置為 `lambda: button_click('+')` 等類似表達式時，實際上是在設置一個將在按鈕被點擊時才調用的回調函數。
