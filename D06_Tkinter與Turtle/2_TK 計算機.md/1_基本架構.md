...
1. 範例中的 `button_click` 函數是通過 `lambda 表達式` 間接參考的，這表示在建立按鈕時，將 command 參數設置為 lambda: button_click('+') 表達式。這樣的設置方式實際上是在指定一個回調函數，該回調函數會在按鈕被點擊時才會執行。lambda 表達式在這裡建立了一個匿名函數，它不會立即調用 button_click，而是將 button_click 函數與按鈕點擊事件綁定。這種設計允許在設置回調函數時傳遞參數，而不需要在初始化時調用函數。
