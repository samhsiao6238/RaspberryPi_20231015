...
2. 由於 `lambda 表達式` 僅在按鈕被點擊時才會被調用，所以即使 button_click 函數 `未被定義` 或 `存在錯誤`，在初始化界面時也不會 `引發錯誤`，這是因為 `lambda 表達式` 並不會立即執行，只是將調用行為 `綁定` 到按鈕的 `點擊事件` 上，這樣當按鈕被點擊時，才會試圖調用 button_click 函數；如果 button_click 函數不存在或定義錯誤，只有在按鈕點擊事件被觸發時才會引發 `NameError 錯誤`，❗️ 但特別注意，這個錯誤不會導致應用程序崩潰，而是僅僅顯示錯誤訊息，這是因為大多數的 `GUI` 框架都會捕捉這些錯誤，並在控制台中顯示錯誤訊息，而不會使應用程式崩潰。
