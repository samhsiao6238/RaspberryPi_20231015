...
1. 導入必要模塊
    ...
    import random
    ```
    - Tkinter 是 Python 的標準 GUI (圖形使用者介面) 程式庫，它封裝了 Tk GUI 工具集的 Python 接口。主要用於創建和管理窗口、按鈕、標籤、文字框、選單、滾動條等等的 GUI 元件。
    - Turtle 模組則是一個使用了 Tkinter 作為實現圖形繪製的庫，所以在使用 Turtle 時，實際上是在一個 Tkinter 窗口中創建了一個畫布，並在該畫布上進行繪圖。
...
1. 說明：簡單示範在 Tkinter 程式中直接使用 Turtle 模組，並將 Turtle 的畫布（Canvas）整合到 Tkinter 的窗口中，這樣便可在 Tkinter 應用程序中利用 Turtle 的繪圖功能。
...
2. 程式碼
    ...
    # 創建一個 Tkinter Canvas 控件
    canvas = tk.Canvas(root)
    # 填滿並且拓展
    canvas.pack(fill=tk.BOTH, expand=True)

    # 創建一個 Turtle 畫筆，並將其與 Tkinter Canvas 關聯
...
2. 程式碼
    ...
    # 創建四個按鈕
    # 使用 ** 將一個字典的鍵值對展開成關鍵字參數，這是一個語法糖用法
    # 特別說明，Tkinter 控件的構造函數允許直接傳遞字典作為配置選項，如同計算機範例
    # 特別注意，不是每一個模組的構造函數都允許這樣的用法
    # 這裡只是使用不同方式來進行語法變化的展示
