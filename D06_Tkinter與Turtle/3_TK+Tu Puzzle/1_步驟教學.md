...
1. 導入必要模組
    ...
    import random
    ```
    - Tkinter 是 Python 的標準 GUI (圖形使用者介面) 程式庫，它封裝了 Tk GUI 工具集的 Python 接口。主要用於創建和管理視窗、按鈕、標籤、文字框、選單、滾動條等等的 GUI 元件。
    - Turtle 模組則是一個使用了 Tkinter 作為實現圖形繪製的庫，所以在使用 Turtle 時，實際上是在一個 Tkinter 視窗中創建了一個畫布，並在該畫布上進行繪圖。
...
2. 自訂函數：添加混亂按鈕
    ...
    def create_scramble_button_tkinter():
        ...
        # 使用 Tkinter 在 Turtle 的畫布上創建一個新視窗，用於放置一個按鈕。
