...
def is_adjacent(el1, el2):
    ...
    return False
```
- index_2d 函數用於找出某個元素在二維列表中的位置。
- swap_tile 函數用於交換玩家點選的圖塊與空白圖塊。
...
```
- is_adjacent 函數檢查兩個圖塊是否相鄰。
- find_empty_square_pos 函數查找空方塊的位置。
- scramble_board 函數用於打亂棋盤。
...
def draw_board():
    ...
    screen.tracer(1)
```
- draw_board 函數用於在 Turtle 畫布上繪製拼圖的圖塊
...
def create_tiles():
    ...
    return board
```
- create_tiles 函數創建拼圖的圖塊並將它們排列在棋盤上。
...
```
- 使用 Tkinter 創建一個按鈕，當點擊時將執行 scramble_board 函數，用於打亂拼圖。
