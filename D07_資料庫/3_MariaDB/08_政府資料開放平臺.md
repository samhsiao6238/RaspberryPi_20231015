...
6. 導入 `pandas`，並透過 `json_normalize` 函數將資料轉換為資料表格式。
...
7. 想從其下 `properties` 的值之中取出其中四個 `on_stop` `sum_of_txn_times` `district_origin` `district_destination`，並分別改名為可識別的鍵。

    ![](images/img_58.png)

<br>

8. 透過 `rename` 函數修改欄位名稱，特別注意，這並非唯一修改的方式。
