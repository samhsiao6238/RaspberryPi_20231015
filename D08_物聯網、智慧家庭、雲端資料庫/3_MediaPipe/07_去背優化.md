...
1. 完整程式碼。
    ...
    def check_if_touched(hand_landmarks, touch_area, width, height):
        ...
        # 假如食指指尖在 80x80 範圍內
        if touch_area[0] < finger_X < touch_area[0] + LENGTH and touch_area[1] < finger_Y < touch_area[1] + LENGTH:
            # 觸發觸碰事件
            return True
        return False
    ...
    with mp_hands.Hands(
        ...
        while True:
            ...
            if is_Touched or frame_count % 10 == 0:
                # 每 20 幀或被觸碰時更新位置
                touch_area = update_touch_area(touch_area, _width, _height, is_Touched)
                frame_count = 0
                # 重置觸碰狀態
                is_Touched = False  
            #
            ...
            # 將蝴蝶圖片放置在觸碰區域
            img = overlay_transparent_image(img, butterfly_image, (touch_area[0], touch_area[1]))
            cv2.imshow('Gesture Touch', img)
            # 按下 q 鍵停止
