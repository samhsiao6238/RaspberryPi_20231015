...
3. 驗證伺服器身份：客戶端驗證伺服器的身份，通常是通過伺服器提供的公鑰進行檢查，以確保連接到的是預期的伺服器而不是冒充的惡意伺服器。
...
1. id_rsa 和 id_rsa.pub：這是一種 RSA 密鑰對。

2. id_dsa 和 id_dsa.pub：這是一種 DSA 密鑰對。

3. id_ecdsa 和 id_ecdsa.pub：這是一種 ECDSA 密鑰對。

4. id_ed25519 和 id_ed25519.pub：這是一種 Ed25519 密鑰對。

5. id_ecdsa_sk 和 id_ecdsa_sk.pub：這是一種使用 FIDO/U2F 安全密鑰的 ECDSA 密鑰對。

6. id_ed25519_sk 和 id_ed25519_sk.pub：這是一種使用 FIDO/U2F 安全密鑰的 Ed25519 密鑰對。
...
2. 這個文件除了可在 VSCode 進行連線外，也可以透過命令行運行，如下示範連線名為 `myserver` 的連線設定。
