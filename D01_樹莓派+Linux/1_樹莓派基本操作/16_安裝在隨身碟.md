...
1. 更新 `EEPROM`，目的是更新 `啟動加載程序（bootloader）` 和相關的配置，特別注意這僅適用於 `樹莓派 4` 以上的型號。
...
3. 可看到以下的設定值預設為 `default`，這表示讓樹莓派可接收所有類型的固件更新，包括測試版、穩定版和關鍵更新；為確保使用的固件版本是穩定且經過充分測試的，從而避免固件的不穩定性所導致的問題，將設定值改為 `critical`。
...
2. 創建並編輯 `autoboot.txt` 文件，特別注意這個文件預設是不存在的，路徑也與前一步驟不同，這文件要放在 `boot` 目錄，在這個範例中就是 `/media/sam6238/bootfs/`。
