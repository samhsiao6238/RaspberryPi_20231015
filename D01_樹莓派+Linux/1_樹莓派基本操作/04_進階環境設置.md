...
1. `config.txt` 是樹莓派設備的配置文件，由樹莓派的 GPU 在 Arm CPU 和 Linux 初始化之前讀取，新版系統位於 `/boot/firmware/` 目錄中，舊版則在 `/boot/` 中。

<br>

2. `config.txt` 每行的字串長度限制為 `98` 個字，超出部分將被忽略。

<br>

3. 完成設置後，必須重啟樹莓派使更改生效。
...
1. 文件包含：可使用 `include` 指令將其他文件的內容包含進當前文件。
...
2. 條件過濾：使用 `autoboot.txt` 文件，這是一個可選配置文件，用於指定啟動分區號和實現 A/B 啟動升級，系統在啟動時會根據配置選擇不同的分區進行啟動，這對於系統升級和故障回滾很有用。
